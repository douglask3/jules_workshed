...
#__version__ = "last known fcm revision: " + filter(str.isdigit, "$Rev: 13195 $")

# these are taken from the jules 4.3 (and adding p_rivers)    
...
VAR_NO_LATLON_DIMS = (# from required_vars_for_configuration at vn4.3
                      'co2_ppmv', 'co2_change_ppmv', 'dtemp_o', 'fa_ocean', 'seed_rain', 
                      'rivers_sto_rp', 'frac_irr_all_tiles', 'irrtiles', 'nirrtile', 'co2_mmr')               
    ...
    * coord_system:
    ...
    if conv_to_grid:
        ...
        while cubelist:
            ...
            if temp_cube.data.dtype.kind in ['S', 'a'] or temp_cube.var_name in VAR_NO_LATLON_DIMS:
        ...
        for i,dim_length in enumerate(cube.shape): 
            if not cube.coords(dim_coords=True, contains_dimension=i):
                if field.dimensions[i] in jules_recognised_dim_names:
                    new_dim_coord = iris.coords.DimCoord(range(dim_length), long_name=jules_recognised_dim_names[field.dimensions[i]])
...
def points_to_grid(pcube, gridfile=None, missingdata=np.nan, latlon_str=None, latlon_flatten_function=None, coord_system=None, **kwargs):
    ...
    * coord_system:
    ...
    not_to_be_copied_over = [len(pcube.data.shape)-1]
    ...
    unnamed_coord_loc = []
    for i in xrange(len(pcube.data.shape)):
    ...
    if (
       ( len(pcube.data.shape) >= 2) and 
       ( pcube.data.shape[-2] == 1 ) and 
       ( len(pcube.data.shape)-2 in unnamed_coord_loc ) and 
       ( 'latitude' in [coord.name() for coord in pcube.aux_coords]) and 
       ( len(pcube.data.shape)-2 in pcube.coord_dims(pcube.coords('latitude')[0]) )
       ):
        data = np.mean(pcube.data, axis=-2) 
        not_to_be_copied_over.append(len(pcube.data.shape)-2)
       ...
       ( pcube.coord_dims(pcube.coord('y')) == (len(pcube.data.shape)-2 ,) ) and 
       ( 'latitude' in [coord.name() for coord in pcube.aux_coords]) and 
       ( len(pcube.data.shape)-2 in pcube.coord_dims(pcube.coords('latitude')[0]) )
       ):
        data = np.mean(pcube.data, axis=-2) 
    ...
    for i in unnamed_coord_loc:
        if not i in not_to_be_copied_over:
            ntiles        = pcube.data.shape[i]
            long_name = 'tiles_or_layers_dim_'+ str(idim)
                
            tilecoord     = iris.coords.DimCoord(range(ntiles), long_name=long_name) # units='no_unit' is for strings only
    ...
    for gridpt in range(data.shape[-1]):
        x = int(latind_int[gridpt])
        y = int(lonind_int[gridpt])
        griddata[..., x, y] = data[..., gridpt]
...
def grid_to_points(cube, lsmask, latlon_str=None, lsmask_missingdata_str='nan', data2D = False, user_def_start_corner=None):  
    ...
    if start_corner == 'topleft':
        ...
        lat_generator = xrange(nlat-1, 0-1, -1)
        lon_generator = xrange(nlon)
        
        if data2D:
            for ilon in lon_generator:
                for ilat in lat_generator: 
                    if mask[ilat, ilon]: 
                        pointsdata[..., 0, ipoint] = cube.data[..., ilat, ilon] 
...
def write_1point_file(cubelist_full, outfile, desired_vars, desired_lonlat, latlon_str=None):
    ...
    * latlon_str:
    ...
    with open(outfile, "w") as outp:
        ...
        for line in xrange(store_number_of_lines):
...
def add_missing_dims(collapsed_cube, cube):
    ...
    new_cube = iris.util.as_compatible_shape(collapsed_cube, cube)
...
def pad_latitude(cube_or_cubelist_in, sample_cube_on_input_grid, latlon_str=None):
    ...
    * latlon_str:
    ...
    while cubelist_in:
        ...
        ymin = np.where(np.isclose(new_cube.coord(lat_str).points, small_cube.coord(lat_str).points[0]))[0]
        ymax = np.where(np.isclose(new_cube.coord(lat_str).points, small_cube.coord(lat_str).points[-1]))[0]
